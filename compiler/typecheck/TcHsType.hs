...
module TcHsType (
        ...
        kindGeneralize, checkExpectedKind,
        reportFloatingKvs,
...
tc_fun_type mode ty1 ty2 exp_kind = case mode_level mode of
  TypeLevel ->
    do { arg_k <- newOpenTypeKind
       ...
       ; checkExpectedKind (ppr $ HsFunTy noExt ty1 ty2) (mkFunTy ty1' ty2')
  ...
  = do { ctxt' <- tc_hs_context mode ctxt
       ...
       ; ty' <- if tcIsConstraintKind exp_kind
                then tc_lhs_type mode ty constraintKind
                        ...
                        ; checkExpectedKind (ppr ty) ty' liftedTypeKind exp_kind }
...
tc_hs_type mode rn_ty@(HsListTy _ elt_ty) exp_kind
  = do { tau_ty <- tc_lhs_type mode elt_ty liftedTypeKind
       ...
       ; checkExpectedKind (ppr rn_ty) (mkListTy tau_ty) liftedTypeKind exp_kind }
...
tc_hs_type mode rn_ty@(HsTupleTy _ HsBoxedOrConstraintTuple hs_tys) exp_kind
  ...
  = do { traceTc "tc_hs_type tuple 2" (ppr hs_tys)
       ...
       ; tys' <- sequence [ setSrcSpan loc $
                            checkExpectedKind (ppr hs_ty) ty kind arg_kind
...
tc_hs_type mode rn_ty@(HsExplicitListTy _ _ tys) exp_kind
  = do { tks <- mapM (tc_infer_lhs_type mode) tys
       ...
       ; checkExpectedKind (ppr rn_ty) ty (mkListTy kind) exp_kind }
  where
...
tc_hs_type mode rn_ty@(HsExplicitTupleTy _ tys) exp_kind
  ...
  = do { ks   <- replicateM arity newMetaKindVar
       ...
       ; checkExpectedKind (ppr rn_ty) (mkTyConApp ty_con (ks ++ taus)) tup_k exp_kind }
  where
    arity = length tys
...
tc_hs_type mode rn_ty@(HsIParamTy _ (L _ n) ty) exp_kind
  = do { MASSERT( isTypeLevel (mode_level mode) )
       ...
       ; checkExpectedKind (ppr rn_ty) (mkClassPred ipClass [n',ty'])
...
tc_hs_type _ rn_ty@(HsStarTy _ _) exp_kind
  ...
  = checkExpectedKind (ppr rn_ty) liftedTypeKind liftedTypeKind exp_kind

--------- Literals
tc_hs_type _ rn_ty@(HsTyLit _ (HsNumTy _ n)) exp_kind
  = do { checkWiredInTyCon typeNatKindCon
       ; checkExpectedKind (ppr rn_ty) (mkNumLitTy n) typeNatKind exp_kind }

tc_hs_type _ rn_ty@(HsTyLit _ (HsStrTy _ s)) exp_kind
  = do { checkWiredInTyCon typeSymbolKindCon
       ; checkExpectedKind (ppr rn_ty) (mkStrLitTy s) typeSymbolKind exp_kind }
...
tc_hs_type _ wc@(HsWildCardTy _) exp_kind
...
tcWildCardOcc wc exp_kind
  = do { wc_tv <- newWildTyVar
       ...
       ; checkExpectedKind (ppr wc) (mkTyVarTy wc_tv)
...
tc_infer_hs_type_ek mode hs_ty ek
  = do { (ty, k) <- tc_infer_hs_type mode hs_ty
       ; checkExpectedKind (ppr hs_ty) ty k ek }
...
finish_tuple rn_ty tup_sort tau_tys tau_kinds exp_kind
  = do { traceTc "finish_tuple" (ppr res_kind $$ ppr tau_kinds $$ ppr exp_kind)
       ...
       ; checkExpectedKind (ppr rn_ty) (mkTyConApp tycon arg_tys) res_kind exp_kind }
  where
      ...
      | otherwise -- if binder is visible
         = case arg of
           (HsValArg ty) -- check the next argument
            -> do { traceTc "tcInferApps (vis normal app)" (vcat [ ppr ki_binder, ppr ty
                                                   , ppr (tyBinderType ki_binder)
                                                   , ppr subst ])
                  ; let exp_kind = nakedSubstTy subst $ tyBinderType ki_binder
                    -- nakedSubstTy: see Note [The well-kinded type invariant]
           ...
           (HsTypeArg ki) -> do { traceTc "tcInferApps (error)" (vcat [ ppr ki_binder, ppr ki
                                                  , ppr (tyBinderType ki_binder)
                                                  , ppr subst, ppr (isInvisibleBinder ki_binder) ])
  ...
  where
    ...
    handle_tyfams tc
      ...
      = do { tc_kind <- zonkTcType (tyConKind tc)
           -- instead of instantiating arguments to tc here, delay until checkExpectedKind
...
unifyKinds rn_tys act_kinds
  = do { kind <- newMetaKindVar
       ; let check rn_ty (ty, act_kind) = checkExpectedKind (ppr $ unLoc rn_ty) ty act_kind kind
